...
func printLongListing(entries []os.DirEntry) {
	for _, entry := range entries {
		...
		if !ok {
			log.Println("Failed to get syscall.Stat_t for entry:", entry.Name())
			continue
		}
		...
		if err != nil {
			log.Println("Failed to lookup user id:", uid, err)
			continue
		}
		groupInfo, err := user.LookupGroupId(strconv.Itoa(gid))
		if err != nil {
			log.Println("Failed to lookup group id:", gid, err)
			continue
		}
		...
		if err != nil {
			log.Println("Failed to stat entry:", entry.Name(), err)
			continue
		}
		fmt.Printf("%s %3d %s %s %6d %s %s\n", mode, stat.Nlink, userInfo.Username, groupInfo.Name, size, timestamp, entry.Name())
	}
}*/
...
func printLongListing(entries []os.DirEntry) {
	...
	for _, entry := range entries {
		...
		timestamp := info.ModTime().Format("Jan _2 15:04")
		fmt.Printf("%s %3d %s %s %6d %s %s\n", mode, info.Sys().(*syscall.Stat_t).Nlink, userInfo.Username, groupInfo.Name, size, timestamp, filepath.Base(entry.Name()))
	}
	fmt.Printf("total %d\n", total)
}
*/
...
func printLongListing(entries []os.DirEntry) {
	for _, entry := range entries {
		...
		fmt.Printf("%s %3d %-8s %-8d %-6s %s %s\n",
			perm, info.Sys().(*syscall.Stat_t).Nlink, info.Sys().(*syscall.Stat_t).Uid, info.Sys().(*syscall.Stat_t).Gid,
			size, modTime, name)
	}
}*/
